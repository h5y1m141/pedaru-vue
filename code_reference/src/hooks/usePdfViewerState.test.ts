...
describe('usePdfViewerState', () => {
  ...
  describe('resetAllState', () => {
    it('should reset all state to default values', () => {
      ...
      act(() => {
        ...
        result.current.setBookmarks([{ page: 5, label: 'Test', createdAt: Date.now() }]);
        result.current.uiSetters.setIsTocOpen(true);
      });
  ...
  describe('refs', () => {
    ...
    it('should have correct initial ref values', () => {
      ...
      expect(result.current.refs.headerWasHiddenBeforeSearchRef.current).toBe(false);
  ...
  describe('pending restore state', () => {
    ...
    it('should update pending restore states through setters', () => {
      ...
      expect(result.current.pendingRestore.pendingTabsRestore).toEqual(tabState);
