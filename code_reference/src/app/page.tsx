...
import type { ViewMode, Bookmark, TabState, WindowState, TocEntry } from '@/types';
...
export default function Home() {
  ...
  async function resolvePageNumber(pdfDoc: any, dest: string | any[] | null): Promise<number | null> {
    if (!dest) return null;
    try {
      let destArray = dest;
  ...
  if (isTauri) return; // Tauriモードでは既にRustで取得済み

  try {
    const outline = await pdfDoc.getOutline();
    if (!outline) {
      setPdfInfo(prev => prev ? { ...prev, toc: [] } : null);
      return;
    }

    // OutlineをTocEntry[]に変換
    const convertOutline = async (items: any[]): Promise<TocEntry[]> => {
      const result: TocEntry[] = [];
      for (const item of items) {
        const page = await resolvePageNumber(pdfDoc, item.dest);
    ...
    const toc = await convertOutline(outline);
    setPdfInfo(prev => prev ? { ...prev, toc } : { title: null, author: null, subject: null, toc });
  } catch (e) {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <ViewerContent
          ...
          onDocumentLoad={(pdfDoc) => {
            handlePdfDocumentLoad(pdfDoc);
            extractTocFromPdf(pdfDoc);
          }}
